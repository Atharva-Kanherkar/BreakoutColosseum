  ...
  useEffect(() => {
    const fetchTournaments = async () => {
      ...
      try {
        ...
        setHostedTournaments(hostedData.items || []) 
        setParticipatingTournaments(participatingData.items || []) 
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-black/60 backdrop-blur-lg border border-red-900/30 p-6 relative">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <Link 
                  href="/tournaments" // Point to the browse page
                  className="flex items-center px-4 py-3 bg-red-800/30 border border-red-900/50 hover:border-red-600 transition-colors"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    {/* Using a different icon might be better, e.g., search or list */}
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /> 
                  </svg>
                  Join Tournament {/* Changed text */}
                </Link> 
            ...
            {tournamentsError ? (
              ...
              <div className="space-y-3">
                {hostedTournaments.map((tournament) => (
                  <Link
                    key={tournament.id}
                    href={`/tournaments/${tournament.id}/host`}
